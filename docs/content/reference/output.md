...
describes what users get. Pick a format with `-o`, or let dataverse choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dataverse <command> -o table   # aligned columns for reading
dataverse <command> -o jsonl   # one JSON object per line, for piping
dataverse <command> -o json    # a single JSON array
dataverse <command> -o csv     # spreadsheet friendly
dataverse <command> -o tsv     # tab-separated
dataverse <command> -o url     # just the URL column
dataverse <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dataverse <command> --fields id,title,url
```
...
```bash
dataverse <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dataverse <command>            # a table, because this is a terminal
dataverse <command> | wc -l    # JSONL, because this is a pipe
```
